...
*/
  while ((character=getc(input)) != (unsigned)EOF)
  {
    ...
    if (code_value[index] != -1) {           /* the table.  If it is,   */
      ...
      printf("Main loop 1: index = %x, code_value = %x, string_code = %x\n",index, code_value[index], string_code);
}
...
printf("Main loop 2: index = %x, code_value = %x, prefix_code = %x, append_character = %x\n",index, code_value[index], prefix_code[index],append_character[index]);
      }
      output_code(output,string_code);  /* When a string is found  */
      string_code=character;            /* that is not in the table*/
printf("Main loop 3: string_code = %x\n",string_code);
...
*/
  ...
  printf("output_code1: string_code = %x\n",string_code);
...
int offset;
  ...
  printf("find match: index = %x, hash_character = %x, hash_prefix = %x, hash_shift = %x\n",index, hash_character, hash_prefix, hash_character << HASHING_SHIFT);
  if (index == 0)
    offset = 1;
  else
  ...
  while (1)
  {
    if (code_value[index] == -1)
     printf("find match2: index = %x, code = %x, hash_prefix = %x\n",index, hash_character, hash_prefix);
      return(index);
    if (prefix_code[index] == hash_prefix && 
        append_character[index] == hash_character)
         printf("find match3: index = %x, code = %x, hash_prefix = %x\n",index, hash_character, hash_prefix);
      return(index);
    index -= offset;
    if (index < 0)
      index += TABLE_SIZE;
...
static unsigned long output_bit_buffer=0L;
  ...
  printf("output_code: code = %x, output_bit_count = %d, output_bit_buffer = %x, output_bit_buffer_shift = %x\n",code, output_bit_count, output_bit_buffer, output_bit_buffer >> 24);
